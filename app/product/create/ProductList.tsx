    ...
    <section className="products-section bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
        {products.map((product, index) => (
          <div key={index} className="border rounded shadow hover:shadow-md transition overflow-hidden">
            <img src={product.image} alt={product.name} className="h-48 w-full object-cover" />
            <div className="p-4">
              <h3 className="font-semibold text-lg">{product.name}</h3>
              <p className="text-sm text-gray-700 line-clamp-3">{product.description}</p>
              <p className="text-orange-600 font-bold mt-2">R$ {parseFloat(product.price).toFixed(2)}</p>
